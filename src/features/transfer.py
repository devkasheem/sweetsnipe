        ...
        try:
            ...
            try:
                ...
                if fee_history and fee_history.get('baseFeePerGas'):
                    ...
                    tx_data['maxFeePerGas'] = int(next_base_fee * 1.5)
                    tx_data['maxPriorityFeePerGas'] = int(priority * 1.5)
                    tx_data.pop('gasPrice', None)
                else:
                    gas_price = await self.w3.eth.gas_price
                    tx_data['gasPrice'] = int(gas_price * 1.1)
            except Exception:
                gas_price = await self.w3.eth.gas_price
                tx_data['gasPrice'] = int(gas_price * 1.1)
        ...
        try:
            ...
            try:
                ...
                if fee_history and fee_history.get('baseFeePerGas'):
                    next_base_fee = fee_history['baseFeePerGas'][-1]
                    priority = fee_history.get('reward', [[0]])[-1][-1] if fee_history.get('reward') else 0
                    tx['maxFeePerGas'] = int(next_base_fee * 1.5)
                    tx['maxPriorityFeePerGas'] = int(priority * 1.5)
                else:
