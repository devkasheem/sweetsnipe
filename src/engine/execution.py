"""
REFACTORED EXECUTION ENGINE with all critical fixes applied:
...
from src.shared.constants import (
    ...
    DEFAULT_GAS_LIMIT,
    PRESIGN_TIME_BEFORE_MINT,
    RPC_RETRY_DELAY
)
from src.shared.circuit_breaker import with_retry_and_timeout, ExecutionTimeoutError
from src.shared.gas_oracle import gas_oracle
from src.shared.validators import validator, ValidationError


class ExecutionUnit:
    def __init__(self, private_key: str, worker_id: int, global_config: ConfigurationManager):
    ...
    async def _rotate_provider(self):
        ...
        Logger.log(self._uid, "WARNING", f"[RPC Rotator] Switching from node {old_index} to {self._current_rpc_index}")
        self._init_web3()
    ...
    def _check_execution_timeout(self):
        ...
        if elapsed >= MAX_EXECUTION_TIME:
            raise ExecutionTimeoutError(
                f"Worker {self._uid} exceeded maximum execution time ({MAX_EXECUTION_TIME}s)"
            )

    async def _guard_gas(self):
        """Check gas price and wait if too high."""
        while True:
            ...
            try:
                if self._uid == 1:
                    ...
                    try:
                        TUI.set_gas_price(current_gas_wei)
                    except:
                        pass
                else:
                    cached = None
                    try:
                        cached = TUI.get_gas_price()
                    except:
                        pass
    ...
    async def run_protocol(self):
        """Main execution protocol with timeout and circuit breaker protection."""
        try:
            # Validate inputs before starting
            try:
                nft_addr_c = validator.validate_ethereum_address(self._cfg.target_nft)
                sea_addr_c = validator.validate_ethereum_address(self._cfg.sea_addr)
                multi_addr_c = validator.validate_ethereum_address(self._cfg.multi_addr)

                if self._cfg.recipient:
                    self._cfg.recipient = validator.validate_ethereum_address(self._cfg.recipient)

                if self._cfg.mint_mode == "DIRECT":
                    validator.validate_mint_function_name(self._cfg.mint_func_name)

            except ValidationError as e:
                Logger.log(self._uid, "FATAL", f"Validation failed: {e}")
                return
            ...
            if self._uid == 1 and self._cfg.verifier_enabled:
                try:
                    ...
                    if not is_safe:
                        raise Exception("Contract Unverified - potential honeypot")
            ...
            if self._uid == 1:
                ...
                start_dt = datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M:%S') if start_time > 0 else "Unknown"
            ...
            Logger.log(
                self._uid,
                "INFO",
                f"Wallet: {self._acct.address[:6]}... | Bal: {bal_eth:.5f} {self._symbol} | {proxy_msg}"
            )
            TUI.update_worker(
                self._uid,
                "INFO",
                balance=f"{bal_eth:.5f}",
                last_msg=f"Ready - Balance: {bal_eth:.5f} {self._symbol}"
            )

            # Handle time sniper and pre-signing
            ...
            if start_time > current_time and not self._cfg.force_start:
                ...
                if self._cfg.presign_enabled and wait_seconds > PRESIGN_TIME_BEFORE_MINT + 2:
                    sleep_prep = wait_seconds - PRESIGN_TIME_BEFORE_MINT
                    if sleep_prep > 0:
                        await asyncio.sleep(sleep_prep)
                    ...
                    if self._cfg.mint_mode == "DIRECT":
                        try:
                            ...
                            tx_build = await func(self._cfg.qty).build_transaction({
                                ...
                                "value": total_value,
                                "nonce": nonce,
                                "gas": self._cfg.presign_gas_limit
                            })
                        except:
                            pass
                    else:
                        tx_build = await multi_contract.functions.mintMulti(
                            self._cfg.qty,
                            nft_addr_c
                        ).build_transaction({
                            "chainId": self._chain_id,
                    ...
                    if tx_build:
                        tx_build = await gas_oracle.apply_gas_strategy(
                            self.w3,
                            self._cfg.rpc_ticker,
                            tx_build,
                            self._cfg.gas_gwei
                        ...
                        Logger.log(self._uid, "SUCCESS", "TX Pre-Signed. Waiting for trigger...")
                ...
                Logger.log(self._uid, "SUCCESS", "WAKE UP! Executing Mint Sequence NOW!")

            # Minting loop with circuit breaker
            first_run = True

            while self._attempt_count < MAX_RETRY_ATTEMPTS:
                self._check_execution_timeout()
                ...
                try:
                    ...
                    TUI.update_worker(
                        self._uid,
                        "INFO",
                        balance=f"{current_bal_eth:.5f}",
                        last_msg=f"Attempt {self._attempt_count}/{MAX_RETRY_ATTEMPTS}"
                    )
                    ...
                    if current_bal_wei < total_value:
                        ...
                        Logger.log(
                            self._uid,
                            "WARNING",
                            f"Insufficient balance. Need {gap:.5f} more {self._symbol}"
                        )
                        await asyncio.sleep(random.uniform(*self._cfg.delay_range))
                        continue

                    Logger.log(self._uid, "INFO", "Initiating Mint Sequence...")
                    nonce = await self._get_nonce()
                    ...
                    if self._cfg.mint_mode == "DIRECT":
                        try:
                            ...
                            Logger.log(
                                self._uid,
                                "FATAL",
                                f"Function '{self._cfg.mint_func_name}' not found on contract"
                            )
                            return
                        ...
                        tx_data = await multi_contract.functions.mintMulti(
                            self._cfg.qty,
                            nft_addr_c
                        ).build_transaction({
                            "chainId": self._chain_id,
                    ...
                    # Apply gas strategy
                    tx_data = await gas_oracle.apply_gas_strategy(
                        self.w3,
                        self._cfg.rpc_ticker,
                        tx_data,
                        self._cfg.gas_gwei
                    ...
                    # Estimate gas
                    tx_data["gas"] = await gas_oracle.estimate_gas_with_buffer(
                        self.w3,
                        tx_data,
                        DEFAULT_GAS_LIMIT
                    )
                    ...
                    Logger.log(self._uid, "WARNING", "Receipt Timeout - transaction may still be pending")
                    self._local_nonce = None
                    ...
                    if any(x in err_str for x in ["429", "503", "connection", "client", "network", "proxy"]):
                        Logger.log(self._uid, "ERROR", f"Connection Error: {err_str[:40]}...")
            ...
            Logger.log(
                self._uid,
                "FATAL",
                f"Max attempts ({MAX_RETRY_ATTEMPTS}) reached. Stopping worker."
            )

        except ExecutionTimeoutError as e:
            Logger.log(self._uid, "FATAL", f"Execution timeout: {e}")
        except Exception as e:
            Logger.log(self._uid, "FATAL", f"Unhandled error: {e}")
